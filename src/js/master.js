...
function parallaxSlides(e){
  ...
  const elements = document.querySelectorAll('.swiper-slide .overlay');
  ...
  $(elements).each((index, element) => {
    ...
    let _hDepth = parseFloat(element.dataset['hDepth']);
    let _vDepth = parseFloat(element.dataset['vDepth']);
    let _large_dataX = parseFloat(element.dataset['lx']);
    let _large_dataY = parseFloat(element.dataset['ly']);
    let _medium_dataX = parseFloat(element.dataset['mx']);
    let _medium_dataY = parseFloat(element.dataset['my']);
    ...
    let pos = `${_dataX + (_mouseX + _w) * _hDepth}% ${_dataY + (_mouseY + _h) * _vDepth}%`;
...
function initQuiz(){
  $('#label-var1').css({
    backgroundImage: `url(/img/quiz/${currentStep}-1.png)`
  });
  $('#label-var2').css({
    backgroundImage: `url(/img/quiz/${currentStep}-2.png)`
  });  
  if($('#quiz-steps').html() == ''){
    for(var f=0; f< totalSteps; f++){
      $('#quiz-steps').append($(`<li class="quiz-step" data-step="${f+1}">${f+1}</li>`));
    }
    $('#quiz-steps').append($(final[langCode]));
  }
    ...
    if(currentStep < totalSteps){
      ...
      $.ajax({
        ...
        success: response => {
          ...
          switch(true){
            case(percentage < 60): conclusionText = conclusions[0][langCode]; $('.quiz-result').addClass('bad');break;
            case(percentage < 80): conclusionText = conclusions[1][langCode]; $('.quiz-result').addClass('med');break;
            case(percentage >= 80): conclusionText = conclusions[2][langCode]; $('.quiz-result').addClass('good');break;
          }
    ...
    //   if($(window).width() >= 600){
        ...
        if(scroll > 0){
            ...
            $('.layer').each((index, el) => {
                ...
                let blur = $(el).data('blur') || 0;
                let offset = scroll * speed;
                if(offset < 0){
                    let offset = 0;
                }
                
                let translate = 'translate3d(-50%, -50%, ' + offset + 'px)';
                let blurVal = (offset / 1000) * blur;
                let blurStr = 'blur(' + blurVal + 'px)';
